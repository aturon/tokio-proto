...
pub trait Server<T: Io> {
    ...
    type Transport: Transport<Item = Self::Request, SinkItem = Self::Response, Error = Self::Error>;
    fn bind_transport(&self, io: T) -> Self::Transport;
}

impl<T: Io, P: Server<T>> BindServer<RpcPipeline, T> for P {
...
impl<'a, T, P> streaming::pipeline::Server<T> for LiftProto<'a, P> where
    T: Io, P: Server<T>
{
